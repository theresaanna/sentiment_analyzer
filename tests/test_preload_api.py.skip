...
class TestPreloadAPI:
    ...
    def test_preload_comments_requires_pro(self, client, auth_headers):
    ...
    def test_preload_comments_success(self, client, pro_user_headers, app):
    ...
    def test_preload_prevents_duplicate_jobs(self, client, pro_user_headers, app):
    ...
    def test_preload_with_invalid_target_comments(self, client, pro_user_headers):
    ...
    def test_preload_caps_at_10000_comments(self, client, pro_user_headers, app):
...
class TestJobsStatusAPI:
    ...
    def test_jobs_status_requires_pro(self, client, auth_headers):
        ...
        with patch('flask_login.utils._get_user') as mock_user:
            ...
            response = client.get('/api/jobs/status', headers=auth_headers)
            assert response.status_code == 402

    def test_jobs_status_returns_user_jobs(self, client, pro_user_headers, app):
        ...
        with app.app_context():
            ...
            response = client.get('/api/jobs/status', headers=pro_user_headers)
    ...
    def test_jobs_status_includes_video_metadata(self, client, pro_user_headers, app):
...
class TestJobCancellationAPI:
    ...
    def test_cancel_job_requires_pro(self, client, auth_headers):
    ...
    @patch('app.cache.enabled', True)
    def test_cancel_job_success(self, mock_redis, client, pro_user_headers):
        ...
        with patch('app.cache.get') as mock_get:
    ...
    @patch('app.cache.redis_client')
    @patch('app.cache.enabled', True)
    def test_cancel_job_not_found(self, mock_redis, client, pro_user_headers):
    ...
    @patch('app.cache.redis_client')
    @patch('app.cache.enabled', True)
    def test_cancel_completed_job_fails(self, mock_redis, client, pro_user_headers):
...
class TestClearOldJobsAPI:
    ...
    @patch('app.cache.redis_client')
    @patch('app.cache.enabled', True)
    def test_clear_old_jobs_success(self, mock_redis, client, pro_user_headers):
        ...
        with patch('app.cache.get') as mock_get:
            ...
            with patch('app.cache.delete') as mock_delete:
                response = client.post('/api/jobs/clear-old',
                                     headers=pro_user_headers)
    ...
    @patch('app.cache.enabled', False)
    def test_clear_old_jobs_cache_disabled(self, client, pro_user_headers):
...
@pytest.fixture
def auth_headers():
    """Return headers for authenticated request."""
    return {'Authorization': 'Bearer test_token'}


@pytest.fixture
def pro_user_headers(app):
    """Return headers for authenticated PRO user request."""
    with app.app_context():
        with patch('flask_login.utils._get_user') as mock_user:
            user = MagicMock()
            user.is_authenticated = True
            ...
            user.id = 1
            mock_user.return_value = user
            return {'Authorization': 'Bearer pro_token'}